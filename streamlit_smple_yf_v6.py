...
class AzureOpenAIAnalyzer:
    ...
    def create_vector_index(self, text: str) -> VectorStoreIndex:
    ...
    def analyze_risks(self, text: str) -> str:
        ...
        # Enhanced prompts based on Overall_risk_v4.py
        risk_sections = {
            ...
            Provide a comprehensive analysis in 2-3 sentences that includes:
            1. Total debt, % of variable-rate debt, and any upcoming maturities
            2. Key debt facilities, interest rate hedging (if any), and covenant requirements  
            3. Risks to liquidity, access to capital, or refinancing pressures
            
            Write as a continuous narrative, not bullet points.""",
            
            "Debt maturity": """From the given 10-Q filing, provide a 2-3 sentence narrative covering:
            Debt maturity including total debt, near-term maturities, long-term maturities, interest rates, principal amount, carrying amount, and repayment or refinancing details. If unavailable, provide details on credit agreements, credit facilities, or any other debt-related risks.
            
            Include specific dollar amounts and dates where available. Write as continuous text, not bullet points.""",
            
            "Interest Expense": """Analyze the 10-Q filing and provide a 2-3 sentence summary about the company's debt interest expenses or interest rates. Include Net Interest expenses of that quarter and same quarter of previous year if available. Include specific dollar amounts and percentages.
            
            If the 10-Q filing does not contain this information, respond exactly with: "This details are not provided in 10-Q filing"
            
            Write as continuous text, not bullet points.""",
            
            "Executive Summary": """Analyse how financial risk factors could negatively impact operations and cashflow presented in the latest 10-Q filing.
            
            Provide a 3-4 sentence narrative summary identifying how financial and debt risk factors, operational risk factors, and any other risks could negatively impact operations and cashflow. Include specific figures where applicable (e.g., $4,900 thousand or $4.9 million) or coverage metrics.
            
            Focus on the most material risks and their potential impact on the company's financial position. Write as continuous prose, not bullet points."""
        }
        ...
        try:
            ...
            # Get currency scale
            currency_response = self.query_vector_index(
                risk_sections["Currency Scale"],
                similarity_top_k=3
            )
    ...
    def analyze_liquidity(self, text: str) -> str:
        ...
        # Define the four specific prompts from Liquidity_summary_v2.py
        prompts = {
            "cash_equivalents": """
    ...
    def analyze_profitability(self, text: str) -> str:
        ...
        # Profit profile prompt from profit_profile.py
        prompt = """
...
def generate_print_report(ticker: str, all_data: Dict) -> str:
    ...
    <body>
        ...
        <p><strong>Analysis Method: Vector Embeddings with Semantic Search</strong></p>
...
def main():
    """Main Streamlit app"""
    st.title("Financial Statement Analyzer with AI & Vector Embeddings")
    ...
    if filing_source == "Upload custom file":
        ...
        if uploaded_file:
            ...
            with st.spinner("Extracting text from uploaded document..."):
                raw_text = analyzer.extract_text_from_file(uploaded_file)
            if raw_text:
        ...
        if ticker:
            try:
                # Load CIK mapping JSON
                with open("C:\\Users\\kumba\\Documents\\Allegis_credit_AI\\Streamlitapp_for_financial_analysis\\company_tickers_exchange.json", "r") as f:
        ...
        try:
            ...
            if not raw_text and pdf_file:
                with st.spinner("Extracting text from 10-Q document..."):
                    raw_text = analyzer.extract_text_from_file(pdf_file)
